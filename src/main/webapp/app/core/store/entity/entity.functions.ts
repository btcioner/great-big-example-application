...
export function loadEntity<T>(state: Entities<T>, action: EntityActions.Add<T> | EntityActions.Load<T>): Entities<T> {
...
/**
 * Called to add a blank record to the store so UI can be displayed
 */
export function addTemp<T>(state: Entities<T>, action: EntityActions.AddTemp<T> | EntityActions.AddOptimistically<T>): Entities<T> {
    const entities = Object.assign({}, state.entities);
    entities[action.payload.id] = reduceOne(state, null, action);
    return Object.assign({}, state, {
        ids: Object.keys(entities),
...
export function deleteEntity<T>(state: Entities<T>, action: EntityActions.Delete<T> | EntityActions.DeleteTemp<T>): Entities<T> {
    ...
    const selectedEntityId = state.ids.indexOf(state.selectedEntityId) > -1 ? null : state.selectedEntityId;
